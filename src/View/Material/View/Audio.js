...
class Audio extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      defaultGroups: [
        {
          _id: '0',
          name: '全部音频',//60 119 120 158 159 141
          default: true,
        },
        {
          _id: '1',
  ...
  render() {
    ...
    selectedItem = selectedItem ? selectedItem : {_id: '0', name: '全部音频'}; 
          ...
          <Sider width={200} style={{minHeight: 280, backgroundColor: '#fff'}}>
            <Menu mode="vertical" defaultSelectedKeys={['0']} selectedKeys={[selectedItem._id]} style={{borderRightWidth: '0', textAlign: 'center'}} onSelect={this.onSelectGroup}>
              ...
              <PopoverModel 
                ...
                confirm={()=>(
                  createGroupName ?
                  createGroup(createGroupName, 'audio') :
...
export default connect(
  state => {
    const groups = state.material.groups.filter(g => g.type === 'audio');
    const material = state.material.material.filter(p => p.type === 'audio');
